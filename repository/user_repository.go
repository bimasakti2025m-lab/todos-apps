...
func (u *userRepository) Create(user model.UserCredential) (model.UserCredential, error) {
	_, err := u.db.Exec("INSERT INTO mst_user (username, password, role) VALUES ($1, $2, $3)", user.Username, user.Password, user.Role)
	if err != nil {
		return model.UserCredential{}, fmt.Errorf("failed to create user: %w", err)
	}
...
func (u *userRepository) GetByUsername(username string) (model.UserCredential, error) {
	...
	if err != nil {
		return model.UserCredential{}, fmt.Errorf("failed to get user by username: %w", err)
	}
	return user, nil
}
